...
const stampedeEvents = {
  ...
  '2025-07-05': ['Shania Twain (Saddledome)', 'Badlands: RL Grime & Rudimental'],
...
export default function TripPlanner() {
  ...
  const updateTripData = (field: string, value: any) => {
    ...
    if (tripData.destinations.length > 2) basePrice += 100 * (tripData.destinations.length - 2)
  ...
  const renderStep = () => {
    switch(step) {
      case 1:
        return (
          <div>
            <h3 className="text-2xl font-bold text-white mb-6">Select Your Date</h3>
            <input
              type="date"
              min="2025-07-03"
              max="2025-07-13"
            ...
            />
            {tripData.date && stampedeEvents[tripData.date as keyof typeof stampedeEvents] && (
              <div className="mt-4 p-4 bg-gold/10 rounded-lg border border-gold/30">
                <p className="text-sm font-bold text-gold mb-2">Events on this day:</p>
                <ul className="text-sm text-gray-300">
                  {stampedeEvents[tripData.date as keyof typeof stampedeEvents].map((event, idx) => (
                    <li key={idx}>• {event}</li>
                  ))}
                </ul>
              </div>
      ...
      case 2:
        return (
          <div>
            <h3 className="text-2xl font-bold text-white mb-6">Choose Pickup Location</h3>
            <input
              type="text"
      ...
      case 3:
        return (
          <div>
            <h3 className="text-2xl font-bold text-white mb-6">Select Destinations</h3>
            <div className="space-y-2">
              {popularLocations.map((location) => (
                <label key={location} className="flex items-center p-3 bg-gray-800 rounded-lg hover:bg-gray-700 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={tripData.destinations.includes(location)}
                    onChange={(e) => {
                      if (e.target.checked) {
                        updateTripData('destinations', [...tripData.destinations, location])
                      } else {
                        updateTripData('destinations', tripData.destinations.filter(d => d !== location))
                      }
                    }}
                    className="mr-3"
                  />
      ...
      case 4:
        return (
          <div>
            <h3 className="text-2xl font-bold text-white mb-6">Party Size & Add-Ons</h3>
            <div className="mb-6">
              ...
              <input
                ...
                onChange={(e) => updateTripData('partySize', parseInt(e.target.value))}
                className="w-full p-4 bg-gray-800 text-white rounded-lg border border-gray-700 focus:border-gold focus:outline-none"
              />
            </div>
            <div className="space-y-2">
              {[
                ...
                { id: 'photographer', label: 'Professional Photographer (+$150)' },
                { id: 'redcarpet', label: 'Red Carpet Arrival (Complimentary)' }
              ].map((addon) => (
                <label key={addon.id} className="flex items-center p-3 bg-gray-800 rounded-lg hover:bg-gray-700 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={tripData.addOns.includes(addon.id)}
                    onChange={(e) => {
                      if (e.target.checked) {
                        updateTripData('addOns', [...tripData.addOns, addon.id])
                      } else {
                        updateTripData('addOns', tripData.addOns.filter(a => a !== addon.id))
                      }
                    }}
                    className="mr-3"
                  />
      ...
      case 5:
        return (
          <div>
            <h3 className="text-2xl font-bold text-white mb-6">Review Your Trip</h3>
            <div className="bg-gray-800 rounded-lg p-6 space-y-4">
              <div>
                <p className="text-sm text-gray-400">Date</p>
                <p className="text-white">{new Date(tripData.date).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>
              </div>
              <div>
              ...
              <div className="pt-4 border-t border-gray-700">
                <p className="text-2xl font-bold text-gold">Estimated: ${estimatedPrice}</p>
                <p className="text-sm text-gray-400 mt-1">Final price confirmed at booking</p>
              </div>
            </div>
          </div>
        )
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex justify-between mb-2">
            {['Date', 'Pickup', 'Destinations', 'Details', 'Review'].map((label, idx) => (
              <span
                key={idx}
                className={`text-sm ${idx + 1 <= step ? 'text-gold' : 'text-gray-500'}`}
              >
                {label}
              </span>
            ))}
